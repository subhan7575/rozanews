    ...
    <div className={`min-h-screen flex flex-col font-sans transition-colors duration-500 ${isVideoPage ? 'bg-black' : 'bg-gray-50 dark:bg-dark'}`}>
      ...
      {!isVideoPage && (
        <div className="w-full px-4 mb-8 relative z-40">
          <header className="mx-auto max-w-7xl rounded-full bg-white dark:bg-dark-lighter shadow-lg border border-gray-100 dark:border-white/5 py-3 px-6 mt-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-2 md:space-x-4">
                <Link to="/videos" className="hidden md:flex items-center justify-center w-10 h-10 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 text-gray-600 dark:text-gray-300 transition-colors">
                  <PlayCircle size={20} />
                </Link>
                
                <button onClick={toggleTheme} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-gray-100 dark:hover:bg-white/10 text-gray-600 dark:text-gray-300 transition-all">
                ...
                {currentUser ? (
                  ...
                  <button onClick={() => setIsAuthOpen(true)} className="hidden md:flex items-center bg-black dark:bg-white text-white dark:text-black px-4 py-2 rounded-full font-bold text-sm shadow-md hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors">
                     Sign In
                  </button>
                )}
      ...
      {/* Mobile Menu */}
      {!isVideoPage && (
        <div className={`fixed inset-0 bg-white/95 dark:bg-dark/95 backdrop-blur-xl z-50 transition-all duration-500 lg:hidden flex flex-col ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`}>
          ...
          <div className="flex-1 flex flex-col justify-center px-8 space-y-8">
              <form onSubmit={handleSearch} className="relative w-full">
                <input
                  type="text"
                  placeholder="Search..."
                  className="w-full py-4 bg-transparent border-b-2 border-gray-200 dark:border-gray-800 outline-none text-3xl font-bold text-gray-900 dark:text-white"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </form>
              <nav className="flex flex-col space-y-4">
                 <Link to="/" onClick={() => setIsMenuOpen(false)} className="text-4xl font-black text-gray-900 dark:text-white">Home</Link>
                 {CATEGORIES.map(cat => (
                   <Link key={cat} to={`/category/${cat}`} onClick={() => setIsMenuOpen(false)} className="text-4xl font-black text-gray-900 dark:text-white">{cat}</Link>
                 ))}
                 <Link to="/weather" onClick={() => setIsMenuOpen(false)} className="text-4xl font-black text-blue-500">Weather</Link>
                 <Link to="/videos" onClick={() => setIsMenuOpen(false)} className="text-4xl font-black text-primary">Videos</Link>
              </nav>
              {currentUser ? (
                <div onClick={() => { navigate('/profile'); setIsMenuOpen(false); }} className="flex items-center gap-4 pt-8 border-t border-gray-200 dark:border-gray-800">
                  <img src={currentUser.avatar} className="w-12 h-12 rounded-full" alt="User" />
                  <div>
                ...
                <button onClick={() => { setIsAuthOpen(true); setIsMenuOpen(false); }} className="w-full bg-primary text-white py-4 rounded-xl font-bold text-xl">Sign In</button>
              )}
          </div>
        </div>
      )}
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-16 mb-20">
              <div className="lg:col-span-4 space-y-8">
                ...
                <div className="flex gap-4">
                   {socialLinks.map((item, i) => (
                      <a key={i} href={item.href} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-white/5 hover:bg-primary flex items-center justify-center transition-all">
                        <item.icon size={20} className="text-gray-400 hover:text-white"/>
                      </a>
                   ))}
                </div>
              </div>
              ...
              <div className="lg:col-span-2">
                ...
                <ul className="space-y-4">
                  {CATEGORIES.slice(0, 5).map(c => (
                    <li key={c}><Link to={`/category/${c}`} className="text-gray-400 hover:text-primary transition-colors">{c}</Link></li>
                  ))}
                  <li><Link to="/weather" className="text-gray-400 hover:text-primary">Weather</Link></li>
                </ul>
              </div>
              ...
              <div className="lg:col-span-2">
                ...
                <ul className="space-y-4">
                  {footerLinks.map(link => (
                    <li key={link.label}><Link to={link.path} className="text-gray-400 hover:text-white">{link.label}</Link></li>
                  ))}
                </ul>
              </div>
              ...
              <div className="lg:col-span-4">
                ...
                <p className="text-gray-400 mb-6">Top stories delivered to your inbox every morning.</p>
                <form className="relative" onSubmit={handleNewsletterSubmit}>
                   ...
                   />
                   <button type="submit" className="absolute right-2 top-2 bottom-2 bg-primary hover:bg-rose-700 text-white font-bold px-6 rounded-lg transition-colors">Join</button>
                </form>
                {newsletterStatus === 'success' && <div className="text-green-400 text-sm mt-2">Subscribed successfully!</div>}
                {newsletterStatus === 'already' && <div className="text-yellow-400 text-sm mt-2">Already subscribed.</div>}
              </div>
            </div>
            
            <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500">
               <p>&copy; 2025 Roza News. Designed by <span onClick={() => navigate('/login')} className="hover:text-primary transition-colors cursor-pointer font-medium" title="Admin Access">Subhan Ahmad</span>.</p>
