        ...
        <div className={`fixed inset-0 bg-white/98 dark:bg-dark-deep/98 backdrop-blur-xl z-[60] transition-all duration-500 lg:hidden flex flex-col ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`}>
          ...
          <div className="flex-1 flex flex-col justify-center px-8 space-y-10">
              ...
              {currentUser ? (
                ...
                <button onClick={() => { setIsAuthOpen(true); setIsMenuOpen(false); }} className="w-full bg-primary text-white py-5 rounded-2xl font-bold text-xl shadow-xl shadow-primary/30">Sign In to Roza</button>
              )}
          </div>
        </div>
      )}
        ...
        <footer className="bg-[#020617] text-white pt-24 pb-12 border-t border-white/5 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-16 mb-20">
              ...
              <div className="lg:col-span-2">
                ...
                <ul className="space-y-4">
                  ...
                  <li><Link to="/weather" className="text-slate-400 hover:text-white flex items-center group font-bold font-bold"><ChevronRight size={12} className="mr-2 opacity-0 group-hover:opacity-100 transition-all"/>Weather</Link></li>
                </ul>
              </div>
            ...
            <div className="border-t border-white/10 pt-12 flex flex-col md:flex-row justify-between items-center text-sm text-slate-500">
               <p>&copy; {new Date().getFullYear()} Roza News. Digital Journalism Reimagined. Designed by <span onClick={() => navigate('/login')} className="text-slate-400 hover:text-primary transition-colors cursor-pointer font-black" title="Admin Portal">Subhan Ahmad</span>.</p>
