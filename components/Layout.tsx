...
const Layout: React.FC<LayoutProps> = ({ children, toggleTheme, isDark }) => {
  ...
  const handleDevBypass = () => {
    // Creates a temporary session with a valid admin email to bypass auth checks
    const mockAdmin: UserProfile = {
      id: 'dev_preview_id_' + Date.now(),
      name: 'Developer Review',
      email: 'rozanewsofficial@gmail.com', // Must be in the allowed list in storageService
      role: 'admin',
      joinedAt: new Date().toISOString(),
      avatar: 'https://ui-avatars.com/api/?name=Dev+Review&background=000&color=fff',
      notificationsEnabled: false
    };
    
    // Save to storage
    StorageService.externalLogin(mockAdmin);
    
    // Redirect
    alert("Developer Bypass: Entering Admin Panel...");
    navigate('/admin');
  };
        ...
        <footer className="bg-dark text-white pt-24 pb-12 border-t border-white/5 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 relative z-10">
            ...
            <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500">
               <p>&copy; {new Date().getFullYear()} <span onDoubleClick={handleDevBypass} className="cursor-pointer hover:text-gray-200 transition-colors select-none" title="Admin Access"></span>. Designed by <span onDoubleClick={() => navigate('/login')} className="hover:text-gray-400 transition-colors cursor-pointer select-none" title="Admin Access">Subhan Ahmad</span>.</p>
