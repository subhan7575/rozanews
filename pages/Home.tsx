...
const Home: React.FC = () => {
  ...
  // --- EMPTY STATE / NO CONTENT (Prevents infinite spinner) ---
  if (articles.length === 0) {
     return (
        <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-dark">
           <div className="w-24 h-24 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mb-6">
              <Zap size={48} className="text-gray-400" />
           </div>
           <h1 className="text-3xl font-black font-display text-gray-900 dark:text-white mb-2">Welcome to Roza News</h1>
           <p className="text-gray-500 mb-8 max-w-md">The platform is ready, but no articles have been published yet. Sign in as an admin to start creating content.</p>
           ...
           <div className="flex gap-4">
              ...
              <button 
                 ...
                 className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-6 py-3 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
              >
                 Reload
              </button>
           </div>
      ...
      <div className="space-y-12 animate-fade-in pb-20">
        ...
        <div className="relative py-12 md:py-16 rounded-3xl overflow-hidden bg-dark text-white mb-8 mx-0 md:mx-4">
           ...
           <div className="relative z-10 container mx-auto px-6 md:px-8">
             ...
             <p className="text-lg md:text-xl text-gray-300 font-light max-w-xl">Explore the latest stories, in-depth analysis, and breaking updates.</p>
           </div>
        </div>
  ...
  // 2. Select Top 2 for Hero Grid
  const heroArticles = mostViewed.slice(0, 2);
  
  // 3. Latest (Chronological)
  ...
  return (
    <div className="space-y-16 md:space-y-20 animate-slide-up pb-20">
      ...
      {/* 1. Cinematic Hero Grid - Top 2 Most Viewed */}
          ...
          {heroArticles.map((article, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 p-6 md:p-12 flex flex-col justify-end items-start">
                ...
                <div className="flex flex-wrap items-center gap-4 md:gap-6 text-xs md:text-sm text-gray-300 font-bold tracking-wide">
      ...
      <section>
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-10 px-2 gap-4">
          <div>
             ...
             <h2 className="text-3xl md:text-4xl font-display font-black text-gray-900 dark:text-white">Latest Headlines</h2>
          </div>
          <button onClick={() => navigate('/category/World')} className="flex items-center px-6 py-3 rounded-full border border-gray-200 dark:border-gray-700 font-bold hover:bg-gray-100 dark:hover:bg-white/5 transition-all text-sm w-fit">
      ...
      <div className="bg-gray-900 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 text-white relative overflow-hidden">
        {/* Background Accents */}
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8">
             <div className="flex items-center mb-8 md:mb-10">
               ...
               <h2 className="text-2xl md:text-3xl font-display font-bold">Business & Markets</h2>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {business.map(article => (
                 <div key={article.id} onClick={() => navigate(`/article/${article.slug}`)} className="group cursor-pointer">
                    ...
                    <h3 className="text-lg md:text-xl font-bold leading-tight group-hover:text-primary transition-colors mb-2 line-clamp-2">{article.title}</h3>
                    <p className="text-gray-400 text-xs md:text-sm line-clamp-2">{article.summary}</p>
                 </div>
               ))}
             </div>
          </div>
               ...
               {tech.map((article, index) => (
                 <div key={article.id} onClick={() => navigate(`/article/${article.slug}`)} className="cursor-pointer group flex gap-5 items-start">
                    ...
                    <div className="pt-1">
                      ...
                      <h4 className="font-bold text-base md:text-lg leading-snug group-hover:text-gray-300 transition-colors line-clamp-2">{article.title}</h4>
                    </div>
                 </div>
               ))}
            </div>
