...
const Home: React.FC = () => {
    ...
    return (
      <div className="space-y-6 md:space-y-8">
        ...
        <div className="border-b-2 border-gray-200 dark:border-gray-800 pb-4 mb-6">
          <h1 className="text-2xl md:text-3xl font-serif font-bold text-gray-900 dark:text-white capitalize">
            {pageTitle}
          </h1>
          <p className="text-gray-500 mt-1 text-sm md:text-base">{displayArticles.length} articles found</p>
        </div>
  ...
  return (
    <div className="space-y-8 md:space-y-12">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {heroArticle && (
          <div 
            ...
            className="lg:col-span-2 relative h-64 md:h-96 rounded-xl overflow-hidden cursor-pointer group shadow-lg"
          >
            <img 
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent flex flex-col justify-end p-4 md:p-6">
              <span className="bg-primary text-white text-[10px] md:text-xs font-bold px-2 py-1 rounded w-fit mb-2 uppercase tracking-wide">{heroArticle.category}</span>
              <h2 className="text-2xl md:text-4xl font-serif font-bold text-white mb-2 leading-tight drop-shadow-md">{heroArticle.title}</h2>
              <p className="text-gray-200 line-clamp-2 hidden md:block text-sm md:text-base">{heroArticle.summary}</p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-6">
          {sideFeatured.map(article => (
            <div 
              ...
              className="relative h-48 md:h-44 rounded-xl overflow-hidden cursor-pointer group shadow-md"
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent flex flex-col justify-end p-4">
                 <h3 className="text-base md:text-lg font-bold text-white leading-tight">{article.title}</h3>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Latest News Grid */}
      <section>
        <div className="flex items-center justify-between mb-4 md:mb-6 border-b-2 border-gray-200 dark:border-gray-800 pb-2">
          <h2 className="text-xl md:text-2xl font-serif font-bold text-gray-900 dark:text-white">Latest News</h2>
          <button onClick={() => navigate('/category/World')} className="text-primary text-sm font-semibold hover:underline">View All</button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {latest.map(article => (
      ...
      {/* Category Sections - Stacked on Mobile, Grid on Desktop */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <section className="lg:col-span-2">
           <h2 className="text-xl md:text-2xl font-serif font-bold mb-4 border-l-4 border-primary pl-3">Business</h2>
        ...
        <aside>
          <h2 className="text-xl md:text-2xl font-serif font-bold mb-4 border-l-4 border-secondary pl-3">Sports</h2>
          <div className="space-y-4">
             {sports.map(article => (
               <div key={article.id} onClick={() => navigate(`/article/${article.slug}`)} className="cursor-pointer group flex gap-3 md:block">
                  <div className="w-24 h-24 md:w-full md:h-40 overflow-hidden rounded mb-0 md:mb-2 shrink-0">
                    <img src={article.imageUrl} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                  </div>
                  <div>
                    <h4 className="font-bold group-hover:text-primary transition-colors text-sm md:text-base leading-tight">{article.title}</h4>
                    <span className="text-xs text-gray-500 mt-1 block">{new Date(article.publishedAt).toLocaleDateString()}</span>
                  </div>
               </div>
             ))}
          </div>
