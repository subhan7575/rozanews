...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInitialArticles = async () => {
       setIsLoading(true);
       // Initial fetch: first 12 articles
     ...
     return (
        <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-dark-deep">
           <div className="w-24 h-24 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mb-6">
              <Globe size={48} className="text-gray-400 animate-pulse" />
           </div>
           <h1 className="text-3xl font-black font-display text-slate-950 dark:text-white mb-2">Connecting to Roza News</h1>
           <p className="text-slate-600 dark:text-slate-400 mb-8 max-w-md font-medium">Fetching the latest stories for you...</p>
           <div className="w-48 h-1 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
              <div className="w-1/2 h-full bg-primary animate-[marquee_2s_linear_infinite]"></div>
           </div>
        </div>
     );
  }
