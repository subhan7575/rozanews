...
const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'articles' | 'drafts' | 'ads' | 'article_editor' | 'code_editor' | 'api' | 'videos' | 'sync' | 'users' | 'messages' | 'ticker' | 'jobs' | 'applications' | 'seo'>('dashboard');
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-black font-sans overflow-hidden">
      <AdminSidebar activeTab={activeTab} setActiveTab={setActiveTab} isSidebarOpen={isSidebarOpen} setSidebarOpen={setSidebarOpen} draftsCount={draftsCount} handleSystemReset={handleSystemReset} handleLogout={handleLogout} />
        ...
        <div className="max-w-7xl mx-auto p-4 md:p-8 pb-32">
          <header className="flex flex-col md:flex-row md:items-center justify-between mb-10 pl-12 md:pl-0 gap-4">
             <div>
                ...
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Roza Terminal Alpha</p>
             </div>
             ...
             <div className="flex items-center gap-3">
                ...
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-wider ${
                   ...
                   <span>GitHub Sync: {syncStatus.time}</span>
                </div>
          ...
          {activeTab === 'article_editor' && <ArticleEditorTab initialArticle={editingArticle} onSaveComplete={() => setActiveTab('articles')} onCancel={() => setActiveTab('articles')} onSaveDraft={handleSaveDraft} />}
