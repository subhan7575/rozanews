...
import { Mail, Briefcase, GraduationCap, PenTool, FileText, CheckCircle, XCircle, Trash2 } from 'lucide-react';
...
const ApplicationsTab: React.FC = () => {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Are you sure you want to delete this application?")) {
      StorageService.deleteJobApplication(id);
      loadApplications();
    }
  };

  const handleStatusChange = (id: string, newStatus: JobApplication['status']) => {
    StorageService.updateJobApplicationStatus(id, newStatus);
    loadApplications();
  };
        ...
        {applications.map(app => (
          <div key={app.id} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
             ...
             <div className="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                {app.status === 'pending' && (
                   <>
                      <button onClick={() => handleStatusChange(app.id, 'reviewed')} className="flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg text-sm font-bold hover:bg-blue-100 transition-colors">
                         Mark Reviewed
                      </button>
                   </>
                )}
