...
import { 
  ...
  Upload, X, Video, Film, Check, Lock, Loader2, Save 
} from 'lucide-react';
...
const ArticleEditorTab: React.FC<ArticleEditorTabProps> = ({ initialArticle, onSaveComplete, onCancel, onSaveDraft }) => {
  ...
  // Sync initialArticle prop to state when it changes
  useEffect(() => {
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 sticky top-4">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-3 pt-2">
                        ...
                        <label className="flex items-center gap-2 bg-yellow-50 dark:bg-yellow-900/10 p-3 rounded-lg border border-yellow-200 dark:border-yellow-900/30"><input type="checkbox" checked={editingArticle.isPremium || false} onChange={(e) => setEditingArticle({...editingArticle, isPremium: e.target.checked})} className="w-5 h-5 rounded text-yellow-600" /><span className="text-sm font-bold dark:text-white flex items-center"><Lock size={14} className="mr-2"/> Premium</span></label>
                    </div>
