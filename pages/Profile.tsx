...
import { 
  ...
  Cloud, Bookmark, Edit, Camera, Zap, Trash2, Crown, ArrowRight, Briefcase, CheckCircle, Clock, XCircle
} from 'lucide-react';
...
const Profile: React.FC = () => {
  ...
  const handlePaymentSuccess = () => {
     if (user) {
        StorageService.upgradeToPremium(user.id);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black pb-24 pt-4 md:pt-10 px-4 md:px-8 animate-fade-in transition-colors duration-500">
           ...
           <div className="flex flex-col md:flex-row items-center md:items-end -mt-20 md:-mt-24 mb-8 gap-6">
              ...
              <div className="text-center md:text-left mb-2 flex-1 min-w-0">
                 <h1 className="text-3xl md:text-5xl font-black font-display text-gray-900 dark:text-white truncate drop-shadow-sm flex items-center justify-center md:justify-start gap-3">
                    ...
                    {user.isPremium && <Crown size={32} className="text-yellow-400 fill-yellow-400" />}
                 </h1>
                 <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-3">
                    ...
                    {user.isPremium && (
                       <span className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-widest flex items-center shadow-md">
                          <Zap size={14} className="mr-1.5" fill="currentColor"/> Premium Member
                       </span>
                    )}
      ...
      <div className="max-w-5xl mx-auto mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="space-y-8">
            {!user.isPremium ? (
               <div className="bg-gradient-to-br from-gray-900 via-black to-gray-900 rounded-[2rem] p-8 text-white text-center shadow-2xl relative overflow-hidden group">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-yellow-500 blur-[80px] opacity-20 group-hover:opacity-40 transition-opacity"></div>
                  ...
                  <h3 className="text-2xl font-black font-display mb-2">Roza Premium</h3>
                  <p className="text-gray-400 text-sm mb-8 leading-relaxed">
                     Unlock ad-free reading, exclusive deep-dive reports, and early access to new features.
                  </p>
                  
                  <button 
                     onClick={() => setShowPayment(true)}
                     className="w-full bg-white text-black font-bold py-4 rounded-xl hover:bg-gray-200 transition-colors shadow-lg flex items-center justify-center"
                  >
                     Upgrade Now <ArrowRight size={18} className="ml-2 group-hover:translate-x-1 transition-transform" />
                  </button>
               </div>
            ) : (
               <div className="bg-gradient-to-br from-yellow-500 to-orange-600 rounded-[2rem] p-8 text-white text-center shadow-2xl relative overflow-hidden">
                  <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-4 backdrop-blur-md">
                     <Crown size={32} />
                  </div>
                  <h3 className="text-xl font-black mb-1">Premium Active</h3>
                  <p className="text-white/80 text-sm">Thank you for your support!</p>
               </div>
            )}
