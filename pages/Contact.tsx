...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Save locally immediately (Guarantees Admin sees it in Dashboard)
    ...
    try {
      await fetch("https://formsubmit.co/ajax/rozanewsofficial@gmail.com", {
        method: "POST",
        headers: { 
        ...
        body: JSON.stringify({
          ...
          email: currentUser.email, 
          message: message,
          _subject: `Contact: ${currentUser.name} - Roza News`,
          _template: "table",
          _captcha: "false"
        })
      });
    } catch (error) {
      console.warn("Email API unreachable, but message saved to database.", error);
    }

    // Always succeed for user since we saved it locally
    setStatus('success');
    setMessage('');
  };
