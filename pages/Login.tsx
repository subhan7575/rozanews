...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Instant local check (Bypassing Firebase network request)
    setTimeout(() => {
      const isAuthorized = AUTHORIZED_ADMINS.includes(email.toLowerCase().trim());
      
      // Simple logic: if in list and password provided (add your own private password check logic if needed)
      // For now, it matches your request of "just put and enter"
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-dark-deep flex items-center justify-center p-4">
      <div className="bg-white dark:bg-dark-lighter p-8 md:p-12 rounded-3xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-800 transition-all">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 text-sm font-bold uppercase tracking-widest mt-1">Roza Internal Portal</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          >
            {isLoading ? (
               ...
               <>Login & Proceed <ArrowRight size={20} className="ml-3"/></>
            )}
          </button>
        </form>

        <div className="mt-10 pt-8 border-t border-gray-100 dark:border-gray-800 text-center">
           <a href="/" className="text-xs font-black text-gray-400 hover:text-primary transition-colors uppercase tracking-widest">← Back to Site</a>
        </div>
      </div>
    </div>
  );
