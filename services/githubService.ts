...
export const GithubService = {
  ...
  generateFileContent: (
    ...
    if (githubConfig?.token) {
      ...
      } else if (githubConfig.token.startsWith("github_pat_")) {
...
export const INITIAL_PROJECT_FILES: VirtualFile[] = ${JSON.stringify(files, null, 2)};`;
  ...
  pushToGithub: async (
    ...
    try {
      ...
      // If not found, check secondary path
      if (getRes.status === 404) {
        ...
        if (getRes.status === 200) {
          ...
          // File doesn't exist anywhere, we'll create it at primary path
          sha = undefined;
        }
      }
      ...
      if (putRes.ok) {
        ...
        if (putRes.status === 404) {
          return { success: false, message: `Repository not found: ${config.owner}/${config.repo}` };
        }
