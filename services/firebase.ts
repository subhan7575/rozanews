...
export const MediaService = {
  /**
   * Primary upload service using Hugging Face
   */
  uploadFile: async (file: File, folder = "uploads"): Promise<string> => {
    try {
      // Direct upload to Hugging Face
      const url = await HuggingFaceService.uploadFile(file, folder);
      return url;
    } catch (err) {
      console.error("Hugging Face upload failed, falling back to local compression", err);
