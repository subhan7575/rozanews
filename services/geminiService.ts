  ...
  generateArticle: async (topic: string, category: string): Promise<Partial<Article>> => {
    ...
    try {
      ...
      return {
        ...
        tags: data.tags,
        imageUrl: `https://picsum.photos/seed/${encodeURIComponent(data.imageSearchQuery?.substring(0, 10) || "news")}/800/600`,
  ...
  generateTags: async (content: string): Promise<string[]> => {
    ...
    try {
      const result = await model.generateContent(
        `Analyze the following text and generate 5-8 relevant, high-traffic SEO tags/keywords. Return valid JSON array of strings only. Example: ["Tech", "AI"].\n\nTEXT: ${content.substring(0, 1000)}`
      );
      const response = await result.response;
