...
// This function combines what's in the browser with what's in the 'All Data' file.
// It uses IDs to ensure we don't have duplicates.
const mergeData = <T extends { id?: string; path?: string }>(localList: T[], serverList: T[], idKey: keyof T = 'id' as keyof T): T[] => {
  const mergedMap = new Map();
  // We prioritize serverList (the All Data file) during an update
  serverList.forEach(item => {
    const key = String(item[idKey]);
    mergedMap.set(key, item);
  });
  // Then we keep any unique local items that weren't in the file yet
...
const init = () => {
  try {
    ...
    // If the file timestamp is newer, it means the user uploaded new code with a data update.
    if (serverTimestamp > localTimestamp) {
      console.log("New data detected in file. Merging...");
      ...
      // Update local timestamp to prevent re-merging
...
export const StorageService = {
  ...
  // Fix: addCommentToArticle used in Article.tsx
  ...
  // Fix: toggleLikeVideo used in Videos.tsx
  ...
  // Fix: addCommentToVideo used in Videos.tsx
  ...
  upgradeToPremium: (userId: string) => {
    ...
    if (idx >= 0) { users[idx].isPremium = true; safeSetItem(KEYS.USERS_DB, JSON.stringify(users)); }
    const current = StorageService.getCurrentUser();
    if (current && current.id === userId) { current.isPremium = true; safeSetItem(KEYS.CURRENT_USER, JSON.stringify(current)); }
  },
  
  // Fix: saveSubscriber used in Layout.tsx
